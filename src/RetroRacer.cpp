...
#include "../ThirdParty/olcConsoleGameEngine.h"


class Racing2D: public olcConsoleGameEngine
{
public:
	Racing2D()
	{
		m_sAppName = L"Racing 2D";
	}
...
protected:
	...
	virtual bool OnUserUpdate(float fElapsedTime)
	{
		...
		while (nTrackSection < static_cast<int>(vecTrack.size()) && fOffset <= fDistance)
		{			
		...
		for (int y = 0; y < ScreenHeight() / 2; y++)
			for (int x = 0; x < ScreenWidth(); x++)
			{
				...
				// Work out segment boundaries
				int nLeftGrass = static_cast<int>((fMiddlePoint - fRoadWidth - fClipWidth) * ScreenWidth());
				int nLeftClip = static_cast<int>((fMiddlePoint - fRoadWidth) * ScreenWidth());
				int nRightClip = static_cast<int>((fMiddlePoint + fRoadWidth) * ScreenWidth());
				int nRightGrass = static_cast<int>((fMiddlePoint + fRoadWidth + fClipWidth) * ScreenWidth());
		...
		int nCarPos = ScreenWidth() / 2 + static_cast<int>((ScreenWidth() * fCarPos) / 2.0) - 7; // Offset for sprite
		...
		{
			int nMinutes = static_cast<int>(t / 60.0f);
			int nSeconds = static_cast<int>(t - (nMinutes * 60.0f));
			int nMilliSeconds = static_cast<int>((t - static_cast<float>(nSeconds)) * 1000.0f);
